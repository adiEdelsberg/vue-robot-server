...
// set PORT=5000 (to set port on terminal)
...
app.get('/api/posts/:year/:month', (req, res) => {
    const response = JSON.stringify(req.params) + ' : ' + JSON.stringify(req.query);
    res.send(response);
});
...
app.put('/api/courses/:id', (req, res) => {
    ...
    if (!course) {
        res.send(404).send('The course with the given Id was not found');
        return;
    }
